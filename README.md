...
**`22일차 : 공지사항에 수정과 삭제기능을 구현하였고, 검색기능을 구현하는 중이다. mongoDB의 필드가 배열이고 배열안의 값이 객체인 값 중에서 그 객체 안에 id값을 가지고 populate하기 위해서는 populate안에 객체로 path값과 populate 하고자 하는 필드를 입력하면 된다.`**