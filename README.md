...
**`38일차 : 구글 로그인 완성, auth 미들웨어를 거치지 않고도 req.user가 있다면 그 유저는 OAuth로 로그인한 유저이기 때문에 auth 미들웨어에 if문으로 바로 next할 수 있도록 하였고 그 밖에 나머지는 Google 로그인과 local 로그인을 구별하여 따로 요청이 갈 수 있도록 설정하였다. Material UI의 Container 컴포넌트는 기본적으로 margin 값이 0 auto 이기 때문에 가운데 정렬을 하게 되어있기 때문에 따로 margin값을 줄 필요가 없을 것 같다.`**